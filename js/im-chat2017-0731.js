(function($, doc) {
	...
	$.plusReady(function() {
		...
		mui.trigger(msg_text_btn,'tap');
		function getData(id, page, pagenum) {
			//获取已有的最新消息ID,避免重复获取
			console.log(document.getElementById("msg-list").innerHTML)
			var having = 1;
						...
						success: function(result) { 
							...
							if(result.data){
								for(var i = 0; i < result.data.length; i++) {
									if(document.getElementById("chat_"+result.data[i].chatid) == undefined){
					...
					if(aid) {
						...
						window.requestAnimationFrame(function(){
							mui.ajax(domain + "/index.php/Api/api/sendMsg", {
								...
								timeout: 1000,
								success: function(result) { 
									console.log("777"+JSON.stringify(result))
									send({
										sender: 'self',
										type: 'text',
										content: val,
						...
						window.requestAnimationFrame(function(){
							...
							mui.ajax(domain + "/index.php/Api/api/sendMsg", {
								...
								success: function(result) {
									...
									//console.log(result.base.domain + (localStorage.getItem("userFace").length<30?"/Uploads/Face/":"") + localStorage.getItem("userFace")) 
								},
								error: function(xhr, type, errorThrown) {
									console.log(errorThrown);
								}
